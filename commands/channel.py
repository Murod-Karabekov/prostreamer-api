...
async def save_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("Kanalni topib bo'lmadi yoki bot admin emas.")
        return ConversationHandler.END
    ...
    await conn.execute(
        ...
        VALUES($1, $2, $3)
        ON CONFLICT (channel_link) DO NOTHING
        """,
        user_id, username, username
    )
    await conn.close()

    await update.message.reply_text(f"Kanal muvaffaqiyatli qo‘shildi: {username}")
    return ConversationHandler.END
